...
# that's how regions look like
  ...
  do.call("grid.arrange", c(plotAMR(noisy.data, NULL, sort(amrs.nonunique)[1:6]), ncol=2))
...
# false positives
  sum(found %outside% c(amrs.unique, amrs.nonunique))

# true negatives
  length(all.ranges %outside% c(amrs.unique, amrs.nonunique))
  
# false negatives
  sum(c(amrs.unique, amrs.nonunique) %outside% found)
```
