  ...
  for (i in seq_along(amr.ranges.relisted)) {
    ...
    if (length(data.hits)>0) {
      ...
      for (j in seq_along(plot.ranges)) {
        plot.data.melt[sample %in% plot.ranges$sample[j] & start %in% GenomicRanges::start( data.ranges[unlist(plot.ranges[j]$revmap)] ),
                       size:=1]
      }

      plot.title <- if (is.null(title)) as.character(GenomicRanges::reduce(plot.ranges, min.gapwidth=window)) else title
